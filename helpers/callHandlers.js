...
const initiateCall = async (req, res) => {
  ...
  try {
    ...
    await notificationService.sendCallMessage(title, message, receiver?.fcm, {
      channelName: credentials?.channelName?.toString(),
      uid: credentials?.uid?.toString(),
      callType: callType?.toString(),
      sessionId: session?._id?.toString(),
      type: "call_invitation",
      callerName: req.user?.firstName?.toString() || "",
      ...
      distance: callerUser?.distance?.toString() || "",
    });
